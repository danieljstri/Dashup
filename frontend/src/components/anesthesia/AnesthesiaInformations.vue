<template>
  <div class="anesthesia-informations">
...
import {
  getMarkupAnesthesiaData,
  getRevenueAnesthesiaData,
} from '@/services/dataService';

export default {
  name: 'AnesthesiaInformations',
  components: {
    DonutChartModel,
    CardModel,
  },
  ...
  methods: {
    async fetchData() {
      try {
        ...
        const dataMarkup = await getMarkupAnesthesiaData(this.selectedMonth);
        ...
        const revenueData = await getRevenueAnesthesiaData(this.selectedMonth);
        const expensesData = await getMarkupAnesthesiaData(this.selectedMonth);
        ...
        if (currentIndex > 0) {
          ...
          const previousRevenueData = await getRevenueAnesthesiaData(previousMonthValue);
          previousRevenueValue = previousRevenueData.value;

          const previousExpensesData = await getMarkupAnesthesiaData(previousMonthValue);
          previousExpensesValue =
            parseFloat(previousExpensesData.value[0]) + parseFloat(previousExpensesData.value[1]);

          previousProfitValue = previousRevenueValue - previousExpensesValue;
